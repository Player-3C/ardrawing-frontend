    ...
    <div className="bg-[#FFF7F5] py-16">
      ...
      <div className="MyContainer">
        ...
        <p className="mt-4 text-center text-gray-700 text-[24px] max-w-6xl mx-auto">
          ...
          images, and draw real-world objects using your phone camera with ease.
        </p>
        ...
        <div className="grid md:grid-cols-3 gap-8 pt-12">
          {features.map((f) => (
            <div
              key={f.title}
              className="h-80 w-md bg-[#FFE9E5] border border-[#FFB3AA] rounded-[22px] shadow px-9 py-12"
            >
              <div className="flex items-center gap-3">
                <f.icon className="text-[#FF6B5E]" />
                <h3 className="text-black text-[28px] font-bold tracking-wide">{f.title}</h3>
              </div>
              <p className="text-gray-700 mt-8 text-[22px] font-normal leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
